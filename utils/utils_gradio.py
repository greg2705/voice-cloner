...
def download_model_hf(model_path : str, url : str) -> bool:
    ...
    try :
        ...
        snapshot_download(repo_id=url, allow_patterns=["*.pth", "*.json", "*.bin", "*.md5","*.py","*.yaml"],local_dir=model_path)
...
def get_audio_from_video(url,output_path,filename):
    ...
    for i in range(10):
        try:
            ...
            pth=audio_video.download(output_path=output_path,filename=filename+audio_video.mime_type.split("/")[-1])
            return pth
